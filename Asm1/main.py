...
def apply_bloom(image):
    ...
    axs[1, 0].imshow(threshold_A, cmap='gray')
    axs[1, 0].set_title('Threshold A')
    axs[1, 0].axis('off')

    threshold_A_blurred = cv2.GaussianBlur(threshold_A, (0, 0), 150).astype(np.float32) / 255
    axs[1, 1].imshow(threshold_A_blurred, cmap='gray')
    axs[1, 1].set_title('Threshold A with Blur')
    axs[1, 1].axis('off')
    ...
    axs[1, 2].imshow(cv2.cvtColor(threshold_A_colored.astype(np.uint8), cv2.COLOR_BGR2RGB))
    axs[1, 2].set_title('Colored Threshold A (Orange)')
    axs[1, 2].axis('off')

    # Threshold B
    _, threshold_B = cv2.threshold(brightness, 0.8 * 255, 255, cv2.THRESH_BINARY)
    axs[2, 0].imshow(threshold_B, cmap='gray')
    axs[2, 0].set_title('Threshold B with Blur')
    axs[2, 0].axis('off')

    threshold_B_blurred = cv2.GaussianBlur(threshold_B, (0, 0), 50).astype(np.float32) / 255
    axs[2, 1].imshow(threshold_B_blurred, cmap='gray')
    axs[2, 1].set_title('Threshold B with Blur')
    axs[2, 1].axis('off')
    ...
    axs[2, 2].imshow(cv2.cvtColor(threshold_B_colored.astype(np.uint8), cv2.COLOR_BGR2RGB))
    axs[2, 2].set_title('Colored Threshold B (Red)')
    axs[2, 2].axis('off')
...
# Load image
image_path = "penguin.jpg"  # Replace with your image path
